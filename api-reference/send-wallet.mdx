...
El `eventName`, `venue` y `dateTime` del frente vienen de la **clase** (no del objeto).
...
En el dashboard admin → detalle del tenant → sección **"Wallet Webhook"**, configura la URL destino.
...
El webhook se dispara de forma asíncrona — no bloquea la respuesta a Apple. Si tu endpoint falla, Axelo loguea el error pero no reintenta.

Solo aplica a **Apple Wallet**. Google Wallet no notifica cuando un usuario elimina un pase — usa `GET /v1/passes/{id}/status` para consultar bajo demanda.
