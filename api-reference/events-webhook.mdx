...
También eventos **de cuenta** de tus dominios propios: `domain.verified` y `domain.verification_failed`
(channel `domain`, sin `job_id`; el `result` trae `{domain, transport, status}`). Los recibes cuando un
dominio pasa de pendiente a verificado (o falla), sin tener que consultar.
